...
10. **mypy does not read pytest's `pythonpath`.** `tests/platform/test_level0_gate.py` imports `level0_gate`, which resolves under pytest but not under `mypy tests substrate` — the exact command CI runs. Caught by reproducing CI locally before pushing, not by CI. Fixed with `mypy_path = "platform"` in `pyproject.toml`, and the workflow now type-checks `platform` too. **Reproduce the CI command verbatim before every push; running a superset locally hides this class of failure.**

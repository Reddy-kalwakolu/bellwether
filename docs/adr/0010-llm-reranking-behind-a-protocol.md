...
- **Heuristic only, no LLM path:** rejected. It would have left the day's central claim weaker than the industry meaning of "reranking", and — more importantly — the LLM path is what exposed the three integration bugs below. Building it paid for itself in findings even though it is not the default.
...
- That degrade path is correct and it is an observability hole. **A completely broken reranker produced entirely plausible numbers.** The only thing that caught it was `hybrid-llm` scoring byte-identical to `hybrid` across every category — two rows that suspiciously agreed to three decimals. Degrade paths need to be loud somewhere, even when they are quiet in the response, so the eval now wraps the reranker in a counter and reports how many queries degraded on every run, zero or not.
- **Three integration bugs were invisible to the hermetic suite and required live calls.** Gemini's `responseSchema` is a restricted OpenAPI dialect that rejects integer `enum` values (400, `TYPE_STRING`), so the 0/1/2 grading schema failed every request; the default model `gemini-2.5-flash` now returns **404 — "no longer available to new users;"** and with both fixed the calls finally landed but the 2,048-token default **truncated the JSON array mid-value on 25 of 26 queries** — twenty full-path `chunk_id`s past a model that spends output budget thinking run to ~5,000 tokens — raising like malformed JSON and degrading just as silently. A fake transport can validate neither a provider's schema dialect, its model catalogue, nor how long its answers run. Model ids now use the `gemini-flash-latest` alias, the grade scale is enforced by the parser rather than the schema, and the rerank call asks for 8,192 output tokens.
...
- **The Claude backend is written and hermetically tested but has never run against the live API.** What is owed is `ANTHROPIC_API_KEY`, not code. Given the three bugs live calls exposed on the Gemini path, the Claude path should be assumed broken until one real request proves otherwise.
